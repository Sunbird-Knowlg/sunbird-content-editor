describe('Assessment service test cases', function() {
    ...
    it("should return questions using search service", function() {
        ...
        EkstepEditor.assessmentService.http.get = jasmine.createSpy().and.callFake(function(url, headers, cb) {
        ...
        spyOn(EkstepEditor.assessmentService, "getQuestions").and.callThrough();
        spyOn(EkstepEditor.searchService, "search").and.callThrough();
        EkstepEditor.assessmentService.getQuestions(data, function(err, res) {
            expect(res).toBe(questions);
            expect(EkstepEditor.assessmentService.getFromService).toHaveBeenCalled();
            expect(EkstepEditor.searchService.search).toHaveBeenCalled();
            expect(EkstepEditor.assessmentService.getFromService.calls.count()).toBe(1);
        });
    });
    ...
    it("should return template using content service", function() {
        ...
        spyOn(EkstepEditor.assessmentService, "getTemplate").and.callThrough();
        spyOn(EkstepEditor.contentService, "getTemplateData").and.callThrough();
        EkstepEditor.assessmentService.getTemplate(templateId, function(err, res) {
            expect(res).toBe(templateData);
            var result = JSON.parse(res);
            expect(result.result.content.identifier).toBe('domain_49060');
            expect(EkstepEditor.contentService.getTemplateData).toHaveBeenCalled();
        });
    });
    ...
    it("should return assessmentitem for given item id", function() {
        ...
        spyOn(EkstepEditor.assessmentService, "getItem").and.callThrough();
        spyOn(EkstepEditor.assessmentService, "getFromService").and.callThrough();
        EkstepEditor.assessmentService.getItem(itemId, function(err, res) {
