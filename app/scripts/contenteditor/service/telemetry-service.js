...
org.ekstep.services.telemetryService = new (org.ekstep.services.iService.extend({
	...
	interact: function (data, options) {
		...
		if (data.duration) {
			eventData.duration = Number((data.duration * 0.001).toFixed(2))
		}
	...
	impression: function (data, options) {
		...
		if (data.duration) { eventData.duration = Number((data.duration * 0.001).toFixed(2)) }
		if (data.subtype) { eventData.subtype = data.subtype }
	...
	*/
	...
	logStartAndImpression: function (config, duration) {
		...
		EkTelemetry.start(config, org.ekstep.contenteditor.api.getContext('contentId'), '', {
			...
			'type': ecEditor.getConfig('editorType') || 'content',
			'mode': mode.toLowerCase(),
			'duration': Number((duration * 0.001).toFixed(2)), // Converting miliseconds to seconds.
			'pageid': 'main-page'
		})
	},
	/**
