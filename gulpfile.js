...
gulp.task('build', ['minify', 'inject', 'packageCorePlugins', 'zip']);
        ...
        if (manifest.editor.dependencies) {
            manifest.editor.dependencies.forEach(function(dependency) {
                if (dependency.type == 'js') {
                    jsDependencies.push('plugins/' + plugin + '/' + dependency.src);
                } else if (dependency.type == 'css') {
                    cssDependencies.push('plugins/' + plugin + '/' + dependency.src);
                }
            });
        }
